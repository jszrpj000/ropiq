...
internal static class Uninstall
{
    ...
    private static void Main(string[] args)
    {
        ...
        string ownDirectory = AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar);
        string target = string.Equals(ownDirectory, expected, StringComparison.OrdinalIgnoreCase) ? ownDirectory : expected;
        if (args.Length == 0)
        {
        ...
        try { if (File.Exists(desktopLauncher)) File.Delete(desktopLauncher); } catch { }
