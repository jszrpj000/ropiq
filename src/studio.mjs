...
function compileVoicePrompt(prompt = {}, job = {}) {
  return [
    promptPart(job.scene_context) && `场景：${promptPart(job.scene_context)}`,
    promptPart(job.emotional_cause) && `情绪原因：${promptPart(job.emotional_cause)}`,
    ...
    promptPart(prompt.emphasis) && `重音：${promptPart(prompt.emphasis)}`,
    promptPart(prompt.tail_tone) && `尾音：${promptPart(prompt.tail_tone)}`,
...
export function compileStagePrompts(stageId, artifact) {
  ...
  if (["voice_synthesis", "product_voice"].includes(stageId)) {
    for (const job of artifact?.execution?.jobs || []) if (job?.voice_prompt) {
      ...
      job.voice_prompt.volume = promptPart(job.voice_prompt.volume || job.voice_prompt.volume_direction) || "自然";
      job.voice_prompt.pace = promptPart(job.voice_prompt.pace || job.voice_prompt.speaking_rate || job.pace) || "自然";
      job.voice_prompt.breath = promptPart(job.voice_prompt.breath || job.voice_prompt.breathing) || "稳定";
